...
infilled_sumry_stats <- met_data %>%
  ...
  summarize(
    prop_soil = sum(soil5_moist_is_infilled) / dplyr::n(),
...
met_data <- met_data %>%
  ...
  ungroup() %>%
  # change scale of soil moisture so looks more similar to the rest
  mutate(soilmoisture_b_5cm_avg_ave = 100 * soilmoisture_b_5cm_avg_ave)


# summary stats on met data for ms table ----------------------------------
...
format_phen <- function(df) {
  ...
  df <- df %>%
    ...
    mutate(
      status = ifelse(yday == doy_event, 1, 0),
      last_day = yday - 1,
      days_since_snow = yday - snowmelt_doy_infilled
    ) %>%
    mutate(
      days_since_snow = ifelse(days_since_snow < 0, 0, days_since_snow),
    ...
    group_by(sensornode, year) %>%
    mutate(
      DD = ifelse(soiltemp_5cm_avg_ave > 0, soiltemp_5cm_avg_ave, 0),
      GDD = cumsum(DD),
...
phen_data_sos_pred <- make_phen_data(sumry, "sos") %>%
  ...
  left_join(., (met_data %>%
    select(
      airtemp_avg_ave, soiltemp_5cm_avg_ave,
      soilmoisture_b_5cm_avg_ave,
      sensornode, ymd, yday, year, snow,
      snowmelt_doy_infilled
    )))
...
# note definitely some things are collinear over certain times of year -
# but retained bc they explain excess variation anyhow
...
# air
sos_air_all <- glm(
  formula = status ~ yday + snowmelt_doy_infilled + temp_7d_air + moist_7d,
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
pop_all <- glm(
  formula = status ~ snowmelt_doy_infilled + moist_7d + yday + temp_7d,
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
pop_air_all <- glm(
  formula = status ~ snowmelt_doy_infilled + moist_7d + yday + temp_7d_air,
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_air_all <- glm(
  formula = status ~ yday + moist_7d + snowmelt_doy_infilled + temp_7d_air,
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
model_cv_surv <- function(phen_data_fit, phen_data_pred, model) {
  out <- list()
...
model_cv_threshold <- function(phen_data_fit, phen_data_pred, thresh) {
  out <- list()
  # drop spatial
  for (i in unique(phen_data_fit$sensornode)) {
    phen_data_fit_i <- phen_data_fit %>%
    ...
    phen_data_pred_i <- phen_data_pred %>%
      filter(sensornode == i)
    mod <- lm(
      formula = as.formula(paste(thresh, paste(1), sep = "~")),
      phen_data_fit_i
    )
...
# full model
eos_fitted <- glm(
  formula = formula(model.aic.backward_eos),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_full_rmse <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted
)


eos_fitted_air <- glm(
  formula = formula(model.aic.backward_eos_air),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
# both spatial and temporal predictions improved by adding yday to the model
eos_full_rmse_air <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_air
)
...
eos_fitted_drop_snowmelt_doy_infilled <- glm(
  formula = update(formula(model.aic.backward_eos), ~ . - snowmelt_doy_infilled),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_snowmelt_doy_infilled_rmse <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_drop_snowmelt_doy_infilled
)
# surv wo moist
eos_fitted_drop_moist_7d <- glm(
  formula = update(formula(model.aic.backward_eos), ~ . - moist_7d),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_moist_7d_rmse <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_drop_moist_7d
)
# surv wo yday
eos_fitted_drop_yday <- glm(
  formula = update(formula(model.aic.backward_eos), ~ . - yday),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_yday_rmse <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_drop_yday
)
# surv wo temp
eos_fitted_drop_temp_7d <- glm(
  formula = update(formula(model.aic.backward_eos), ~ . - temp_7d),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_temp_7d_rmse <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_drop_temp_7d
)

# repeat for air
eos_fitted_drop_yday_air <- glm(
  formula = update(formula(model.aic.backward_eos_air), ~ . - yday),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_yday_rmse_air <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_drop_yday_air
)

# surv wo moist
eos_fitted_drop_moist_7d_air <- glm(
  formula = update(formula(model.aic.backward_eos_air), ~ . - moist_7d),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_moist_7d_rmse_air <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
  eos_fitted_drop_moist_7d_air
)


eos_fitted_drop_snowmelt_doy_infilled_air <- glm(
  formula = update(formula(model.aic.backward_eos_air), ~ . - snowmelt_doy_infilled),
  family = binomial(link = "logit"),
  data = phen_data_eos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
eos_drop_snowmelt_doy_infilled_rmse_air <- model_cv_surv(
  phen_data_eos, phen_data_eos_pred,
...
eos_effects <- plot_conditional_effect(
  dataset = phen_data_eos,
  model = eos_fitted,
...
eos_effects_air <- plot_conditional_effect(
  dataset = phen_data_eos,
...
pop_fitted <- glm(
  formula = formula(model.aic.backward_pop),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
# both spatial and temporal predictions improved by adding yday to the model
pop_full_rmse <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted
)

#
pop_fitted_air <- glm(
  formula = formula(model.aic.backward_pop_air),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
# both spatial and temporal predictions improved by adding yday to the model
pop_full_rmse_air <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_air
)


pop_fitted_drop_snowmelt_doy_infilled <- glm(
  formula = update(formula(model.aic.backward_pop), ~ . - snowmelt_doy_infilled),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
pop_drop_snowmelt_doy_infilled_rmse <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_drop_snowmelt_doy_infilled
)


pop_fitted_drop_yday <- glm(
  formula = update(formula(model.aic.backward_pop), ~ . - yday),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
pop_drop_yday_rmse <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_drop_yday
)


pop_fitted_drop_moist_7d <- glm(
  formula = update(formula(model.aic.backward_pop), ~ . - moist_7d),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
    filter(days_since_snow > 0)
)
pop_drop_moist_7d_rmse <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_drop_moist_7d
)
# air drop 1 term

pop_fitted_drop_snowmelt_doy_infilled_air <- glm(
  formula = update(formula(model.aic.backward_pop_air), ~ .
  - snowmelt_doy_infilled),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
    ungroup() %>%
...
pop_drop_snowmelt_doy_infilled_rmse_air <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_drop_snowmelt_doy_infilled_air
)

pop_fitted_drop_yday_air <- glm(
  formula = update(formula(model.aic.backward_pop_air), ~ .
  - yday),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
...
pop_drop_yday_rmse_air <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_drop_yday_air
)

pop_fitted_drop_moist_7d_air <- glm(
  formula = update(formula(model.aic.backward_pop_air), ~ .
  - moist_7d),
  family = binomial(link = "logit"),
  data = phen_data_pop %>%
...
pop_drop_moist_7d_rmse_air <- model_cv_surv(
  phen_data_pop, phen_data_pop_pred,
  pop_fitted_drop_moist_7d
)
...
# all_rmse_sos = all_rmse_sos %>% data.table::rbindlist(, idcol = 'model')
pop_effects <- plot_conditional_effect(
  dataset = phen_data_pop,
  model = pop_fitted,
...
pop_effects_air <- plot_conditional_effect(
  dataset = phen_data_pop,
...
sos_fitted <- glm(
  formula = formula(model.aic.backward_sos),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
# both spatial and temporal predictions improved by adding yday to the model
sos_full_rmse <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted
)

sos_fitted_air <- glm(
  formula = formula(model.aic.backward_sos_air),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
# both spatial and temporal predictions improved by adding yday to the model
sos_full_rmse_air <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_air
)


sos_fitted_drop_snowmelt_doy_infilled <- glm(
  formula = update(formula(model.aic.backward_sos), ~ . - snowmelt_doy_infilled),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_snowmelt_doy_infilled_rmse <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_drop_snowmelt_doy_infilled
)

sos_fitted_drop_snowmelt_doy_infilled_air <- glm(
  formula = update(formula(model.aic.backward_sos_air), ~ . - snowmelt_doy_infilled),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_snowmelt_doy_infilled_rmse_air <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_drop_snowmelt_doy_infilled_air
)


sos_fitted_drop_temp_7d <- glm(
  formula = update(formula(model.aic.backward_sos), ~ . - temp_7d),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_temp_7d_rmse <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_drop_temp_7d
)

sos_fitted_drop_temp_7d_air <- glm(
  formula = update(formula(model.aic.backward_sos_air), ~ . - temp_7d_air),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_temp_7d_air_rmse_air <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_drop_temp_7d_air
)


sos_fitted_drop_yday <- glm(
  formula = update(formula(model.aic.backward_sos), ~ . - yday),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_yday_rmse <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_drop_yday
)

sos_fitted_drop_yday_air <- glm(
  formula = update(formula(model.aic.backward_sos_air), ~ . - yday),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_yday_rmse_air <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
  sos_fitted_drop_yday_air
)


sos_fitted_drop_moist_7d_air <- glm(
  formula = update(formula(model.aic.backward_sos_air), ~ . - moist_7d),
  family = binomial(link = "logit"),
  data = phen_data_sos %>%
    ungroup() %>%
    filter(days_since_snow > 0)
...
sos_drop_moist_7d_rmse_air <- model_cv_surv(
  phen_data_sos, phen_data_sos_pred,
...
# all_rmse_sos = all_rmse_sos %>% data.table::rbindlist(, idcol = 'model')
sos_effects <- plot_conditional_effect(
...
sos_effects_air <- plot_conditional_effect(
  dataset = phen_data_sos,
...
prow <- cowplot::plot_grid(sos_vs_thresh + theme(
